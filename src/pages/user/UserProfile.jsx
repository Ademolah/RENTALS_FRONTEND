...
import { Heart, Building, Settings, MapPin, LogOut, Clock, ShieldAlert } from 'lucide-react';
...
export const UserProfile = () => {
  ...
  // Intercept the upgrade state from either the router payload or the backend user object
  const isPendingVerification = location.state?.upgradePending || user?.agencyStatus === 'PENDING';
  ...
  return (
    <div className="min-h-screen bg-brand-slate text-white p-6 md:p-10 font-sans">
      <div className="max-w-[1200px] mx-auto space-y-10">
        ...
        {/* =======================================================================
            DYNAMIC CORPORATE UPGRADE SECTOR
            ======================================================================= */}
        {isPendingVerification ? (
          
          /* PENDING STATE CARD */
          <div className="bg-gradient-to-r from-brand-midnight to-[#1a233a] border border-brand-gold/30 rounded-[2rem] p-8 md:p-10 relative overflow-hidden shadow-premium">
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="max-w-xl">
                ...
                <h2 className="text-2xl font-display font-bold text-white mb-2">Corporate Account Pending</h2>
                <p className="text-white/60 text-sm leading-relaxed font-medium">
                  Your agency configuration is currently under strict review by Rentals Super Administrators. Your dashboard will automatically unlock once your CAC metrics are securely validated.
                </p>
              </div>
              <button 
                disabled
                className="shrink-0 bg-white/5 border border-white/10 text-white/40 px-8 py-4 rounded-xl font-bold text-sm cursor-not-allowed flex items-center gap-2"
              >
                <ShieldAlert size={18} />
                Awaiting Clearance
              </button>
            ...
            {/* Abstract Background Graphic (Clock for Pending) */}
